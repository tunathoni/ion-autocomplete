ion-autocomplete
================
> Configurable Ionic directive for an autocomplete dropdown. and a little additional function for disable item and handle with it.
...
Each of items can be disabled with `disabled` parameter in the items
example:
...
```
as you can see that `disabled` = `true` is for disable item.
